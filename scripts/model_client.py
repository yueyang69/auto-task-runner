...
def _call_via_openclaw_cmd(prompt: str, model: str, label: str, timeout: int) -> str:
    """
    模式 3: 直接执行 Python 脚本调用 Gateway API (保底)
    
    注意：openclaw CLI 没有直接的 send 命令，所以保底方案是
    执行一个简单的 Python 脚本，通过 HTTP 调用 Gateway API
    """
    try:
        # 创建临时脚本
        script_file = Path("/tmp/auto_task_call_model.py")
        # 转义 prompt 中的引号
...
# Gateway API (默认端口 19000)
url = "http://localhost:19000/api/v1/chat/completions"
data = json.dumps({{
    "model": model,
...
try:
    ...
    with urllib.request.urlopen(req, timeout={timeout}) as resp:
        ...
        print(result.get("choices", [{{}}])[0].get("message", {{}}).get("content", "[无响应]"))
except Exception as e:
...
'''
        ...
        if result.returncode == 0:
            log(f"✅ {label} - 直接 API 调用成功")
            return result.stdout.strip()
        else:
            log(f"⚠️ {label} - API 调用失败：{result.stderr[:200]}")
            return f"[{model} 调用失败：{result.stderr[:100]}]"
