...
class Watchdog:
    ...
    def _check_task(self, task: dict, all_tasks: list):
        task_id = task["id"]
        hb_file = HEARTBEAT_DIR / f"task_{task_id}.json"

        # 读取心跳文件
        if not hb_file.exists():
            # 没有心跳文件：任务在 running 状态但从未写心跳
            # 可能是旧版本遗留，或启动瞬间崩溃
            ...
            self._recover_task(task_id, all_tasks, pid=None, reason="无心跳文件")
            return
        ...
        try:
            ...
            self._recover_task(task_id, all_tasks, pid=pid, reason="心跳时间格式错误")
            return
        ...
        self._recover_task(task_id, all_tasks, pid=pid,
                           reason=f"心跳超时 {age_minutes:.0f}min")

    def _recover_task(self, task_id: int, all_tasks: list, pid, reason: str):
